...
router.post("/add-transaction", authenticateToken, async (req, res) => {
  ...
  try {
    const existingTransaction = allTransactions.find((txn) => {
      const normalizedExisting = txn.name.replace(/\s+/g, "").toLowerCase();
...
router.put("/edit-transaction/:id", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    if (name && name !== existingTransaction.name) {
      const duplicateName = allTransactions.find((txn) => {
        const normalizedExisting = txn.name.replace(/\s+/g, "").toLowerCase();
        return (
          normalizedExisting === normalizedName && txn._id.toString() !== id // Exclude the current transaction by ID
        );
      });
...
router.post("/generate-queue-number", authenticateToken, async (req, res) => {
  const { transaction, userType } = req.body;
  const db = getDb();

  try {
    const latestQueue = await db
      .collection("queue-numbers")
      .find({ transactionName: transaction?.name })
      .sort({ createdAt: -1 })
      .limit(1)
      .toArray();
    ...
    if (transaction.isIDReset) {
      newNumber = "001";
      await db
        .collection("transactions")
        .updateOne({ name: transaction?.name }, { $set: { isIDReset: false } });
    ...
    const generatedQueuenumber = transaction.transactionID + newNumber + suffix;
